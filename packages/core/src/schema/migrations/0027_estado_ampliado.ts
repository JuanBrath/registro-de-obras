...
// SQLite no permite ALTER de un CHECK existente: hay que reconstruir las
// tablas (mismo patron que 0017_venta_donacion). Se preservan todas las
// columnas y datos tal cual; el unico cambio de esquema es que "estado"
// (tanto en obra como en ejemplar) ahora tambien acepta los nuevos casos.
export const migration0027EstadoAmpliado: Migration = {
  name: "0027_estado_ampliado",
  sql: `
CREATE TABLE obra_new (
  ...
  estado TEXT NOT NULL DEFAULT 'disponible' CHECK (estado IN ('disponible','vendida','reservada','exhibicion','consignacion','destruida','descartada','en_produccion','coleccion_autor')),
...
INSERT INTO texto_ayuda (field_key, texto_es, texto_en) VALUES
  ('estado_ejemplar', 'Consignacion: la pieza esta en manos de un tercero (galeria, feria, deposito) para su eventual venta, pero todavia no se vendio. Destruida: la copia fue destruida y ya no existe fisicamente. Descartada: se descarto por un defecto de impresion u otro motivo, sin llegar a ser una pieza valida. En produccion: todavia se esta imprimiendo o produciendo, no esta terminada. Coleccion del autor: quedo en poder del propio artista, no esta disponible para la venta.', 'Consignment: the piece is with a third party (gallery, fair, storage) for eventual sale, but has not been sold yet. Destroyed: the copy was destroyed and no longer physically exists. Discarded: it was discarded due to a printing defect or other reason, without becoming a valid piece. In production: it is still being printed or produced, not finished. Artist''s collection: it remains with the artist, not available for sale.');
`,
};
