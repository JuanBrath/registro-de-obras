...
/** Opciones comunes a cualquier informe en PDF: idioma elegido, membrete y firma. */
...
/** Arranca un documento A4 con el membrete (logo propio o monograma GS) ya dibujado. */
export async function nuevoDocConMembrete(
  ...
  const marginLeft = 14;
  const startY = await drawPdfHeader(doc, titulo, { marginLeft, logoBytes: opts.logoBytes });
